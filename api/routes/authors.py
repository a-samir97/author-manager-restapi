...
allow_extensions = set(['image/jpeg', 'image/png', 'jpeg'])

def allowed_file(filename):
	return filename in allow_extensions
...
@author_routes.route('/<int:authr_id>', methods=['GET'])
...
def update_author_detail(id):
	...
	get_author.first_name = data['first_name'] if data['first_name'] else get_author.first_name
	get_author.last_name = data['last_name'] if data['last_name'] else get_author.last_name
...
def author_avatar(author_id):
	try:
		...
		author_schema = AuthorSchema()
		author = author_schema.dumps(get_author)
		return response_with(resp.SUCCESS_200, value={"author": author})
